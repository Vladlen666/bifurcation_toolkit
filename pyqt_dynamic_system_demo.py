...
def safe_exp(z):                      # работает со скаляр./массивом
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.traj_lines, self.nullcline_art, self.nullcline_pts = [], [], []
        self.field_art, self.sep_lines, self.equilibria = [], [], []
        ...
        tabs = QTabWidget(); page = QWidget()
        QVBoxLayout(page).addWidget(self.splitter); tabs.addTab(page, "Plots")
        ...
        self._build_toolbar()
        self._configure_param_axes(); self._configure_phase_axes()
    ...
    def _compile_system(self):
        ...
        self.detJ_lam = sp.lambdify((x, y, m1, m2), detJ,
                                    modules=[SAFE_MODULE, 'numpy'])
        self.dtr_dmu1_lam = sp.lambdify((x, y, m1, m2), dtr_dmu1,
                                        modules=[SAFE_MODULE, 'numpy'])
    ...
    def _build_toolbar(self):
        ...
        bar.addSeparator()
        self.act_max_param = QAction("Max Param", self, checkable=True)
        ...
        # кнопка x(t)
        bar.addSeparator()
        self.act_xt = QAction("Plot x(t)", self); self.act_xt.setEnabled(False)
        self.act_xt.triggered.connect(self._plot_xt); bar.addAction(self.act_xt)
    ...
    def _on_spin_changed(self, _=None):
        ...
        self._toggle_separatrices(False)           # УБИРАЕМ СЕПАРАТРИСЫ
    ...
    def _on_param_click(self, e):
        ...
        self._clear_trajectories(); self._toggle_separatrices(False)
        if self.param_marker: self.param_marker.remove()
        self.param_marker, = self.param_canvas.ax.plot(μ1, μ2, "xr", ms=8)
        self.param_canvas.canvas.draw_idle()
        self._draw_nullclines(μ1, μ2); self._draw_vector_field() if self.show_field else None
    ...
    # ----------------------------------------------------------------------- #
    #                    6.  Двух-этапный поиск равновесий                    #
    # ----------------------------------------------------------------------- #
    def _find_equilibria_numeric(self, μ1, μ2):
        sols: list[tuple[float, float]] = []
        tol_f = 1e-4      # допустимый ноль функции
        tol_xy = 1e-3     # минимальное расстояние между решениями
        ...
        for x0, y0 in guesses:
            ...
            if sol.success:
                x_eq, y_eq = sol.x
                if max(abs(fg((x_eq, y_eq))[0]), abs(fg((x_eq, y_eq))[1])) > tol_f:
                    continue
                if any(np.hypot(x_eq - xs, y_eq - ys) < tol_xy for xs, ys in sols):
                    continue
                sols.append((x_eq, y_eq))

        # --- 2. Усилитель: ищем точки, где |f| и |g| малы на сетке 50×50 -----
    ...
    # ----------------------------------------------------------------------- #
    #                    8.  Векторное поле и сепаратрисы                     #
    # ----------------------------------------------------------------------- #
    ...
    def _draw_vector_field(self):
        ...
        Q = ax.quiver(XX, YY, U / M, V / M, angles="xy",
                      pivot="mid", alpha=0.6)
    ...
    def _draw_hopf(self):
        ...
        detJ_lam = sp.lambdify((x, y, μ1, μ2), sp.simplify(J.det()),
                               modules=[SAFE_MODULE, 'numpy'])
        dtr_dmu1_lam = sp.lambdify((x, y, μ1, μ2), sp.diff(trJ_sym, μ1),
                                   modules=[SAFE_MODULE, 'numpy'])
        dtr_dmu2_lam = sp.lambdify((x, y, μ1, μ2), sp.diff(trJ_sym, μ2),
                                   modules=[SAFE_MODULE, 'numpy'])
        curves = []
        for sol in eqs:
            xi, yi = sol[x], sol[y]
    ...
    def _on_phase_click(self, e):
        ...
        # сохраняем t и x(t) для Plot x(t)
        self.last_t = np.concatenate([sol_b.t[::-1][:-1], sol_f.t])
        self.last_x = xs; self.act_xt.setEnabled(True)

    def _plot_xt(self):
        if not hasattr(self, "last_t"):
            QMessageBox.information(self, "x(t)", "Сначала постройте траекторию в фазовой плоскости.")
            return
        fig, ax = plt.subplots()
        ax.plot(self.last_t, self.last_x)
        ax.set_xlabel("t"); ax.set_ylabel("x(t)")
        ax.set_title("Time trace x(t)"); fig.show()

    # ----------------------------------------------------------------------- #
    #                      11.  Прочие вспомогательные                        #
    # ----------------------------------------------------------------------- #
